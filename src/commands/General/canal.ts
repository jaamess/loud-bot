...
export default class extends Command {
    ...
    public async remove(message: KlasaMessage, channel: GuildChannel): Promise<KlasaMessage | KlasaMessage[]> {
        ...
        // @ts-ignore
        if (!whitelistedChannels.includes(channel.id)) { return message.send(`<:loudwarning:591525783994892288>  **  |  O canal de nome ${channel.name} não está registrado no banco de dados, logo, não é possível removê-lo.**`); }
