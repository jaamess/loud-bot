...
import { TesseractExtendWorker } from '../../src/lib/structures/TesseractExtendedWorker';
import Cropper from '../../src/lib/util/CanvasCropper';
const worker = new TesseractExtendWorker();

export default class extends Monitor {
    private NUMBER_REGEX: RegExp;
    private DETECTION_ATTEMPTS: number;
    ...
    public async run(message: KlasaMessage): Promise<void> {
        ...
        if (!screenshot.length) {
            if (message.author.id !== '254808241643585547') {
                message.delete();
                const response = await message.send('<:loudwarning:591525783994892288>  |  **Você não enviou uma captura de tela válida para verificar sua patente. Tente novamente.**') as KlasaMessage;
                setTimeout(() => {
                    response.delete();
                }, 5000);
            }
        ...
        const working = await message.send('Verificando patente...') as KlasaMessage;
        let score = await this._parseImage(screenshot);
        if (!score) score = await this._parseImage(screenshot);
        ...
        const rank = await this._parseRank(score);
        ...
        this._giveRole(message, rank);
    }

    // Crops the image and recognises the characters in the image
    private async _parseImage(image: string[]): Promise<number> {
    ...
    private _parseRank(score: number): string {
        let rank = '';
    ...
    private _giveRole(message: KlasaMessage, rank: string): Promise<GuildMember> {
        let role;
    ...
    private async _addRole(message: KlasaMessage, rank: string): Promise<GuildMember> {
        const GUILD_LOUD = {
        ...
        if (message.guild.me.roles.highest.position < message.guild.roles.get(rank).position) {
            await message.send(`<:loudwarning:591525783994892288>  **|  O meu cargo não é alto o suficiente para realizar esta ação.**`);
            return null;
        }
