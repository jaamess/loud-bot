...
module.exports = class extends Event {
	...
	async run(member) {
		...
		if (brunoBot.includes(message.member.username)) {
			...
			member.guild.channels.get('579470954166747146').send(`:loudwarning: * |  Self bot do PH detectado e banido. Usuario: ${member.user.username}#${member.user.discriminator}. ID: ${member.id}`);
